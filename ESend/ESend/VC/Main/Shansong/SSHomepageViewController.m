...
@interface SSHomepageViewController ()<UINavigationControllerDelegate,UITextFieldDelegate,ABPeoplePickerNavigationControllerDelegate,SSAppointmentTimeViewDelegate,BMKLocationServiceDelegate,BMKGeoCodeSearchDelegate>{
...
@property (weak, nonatomic) IBOutlet UIButton *hp_tipBtn;

// 取货时间
@property (nonatomic,assign) BOOL api_pick_now;
@property (nonatomic,copy) NSString * api_pick_time;
@property (weak, nonatomic) IBOutlet UIButton *hp_pickTimeBtn;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *hp_pickTimeBtnWidth;
...
- (IBAction)selectTips:(UIButton *)sender {
    NSLog(@"%s",__func__);
}
- (IBAction)selectPickTime:(UIButton *)sender {
    NSLog(@"%s",__func__);
}
...
- (IBAction)editAddress:(UIButton *)sender {
    ...
    SSAddrInfoVC * aivc = [[SSAddrInfoVC alloc] initWithNibName:@"SSAddrInfoVC" bundle:nil];
    aivc.addrType = type;
