...
@implementation OrderDetailReciverTableCell
{
    UIView * _bgView;
...
- (void)loadData:(id)data{
    ...
    SupermanOrderModel * model = (SupermanOrderModel *)data;
    
    if (isCanUseString(model.receivePhone)) {
        _phoneNoLabel.text = model.receivePhone;
    } else {
        _phoneBgIcon.hidden = YES;
    }
    
    
    _nameLabel.text = model.receiveAddress;
...
+ (CGFloat)calculateCellHeight:(id)data{
    ...
    CGSize size = [Tools stringHeight:model.receiveAddress fontSize:NormalFontSize width:(ScreenWidth - Space_Big - 70 - Space_Big)];
    
    if (isCanUseString(model.receivePhone)) {
        return Space_Big + size.height + Space_Big + 40 + Space_Big;
    } else {
        return Space_Big + size.height + Space_Big + 40 + Space_Big;
    }
